        ...
        const fetchData = async () => {
            try {
                const [ls, orders, overview] = await Promise.all([
                    ...
                    fetch((process.env.REACT_APP_API_URL || 'http://localhost:8000/api') + '/dashboard/overview', {
        ...
        <div className="container-fluid">
            <div className="row mb-3">
                { [
                    ...
                    { label: 'Active Orders', value: totals.active_orders },
                    { label: 'Inventory Items', value: totals.inventory_items },
                    { label: 'Customers', value: totals.customers }
                ].map((stat, idx) => (
                    <div className="col-md-3 mb-3" key={idx}>
                        <div className="card p-3">
                            <div className="fw-bold">{stat.label}</div>
                            <div className="fs-4">{stat.value}</div>
                        </div>
                    </div>
                )) }
            </div>
            ...
            <div className="row mb-3">
                <div className="col-lg-6 mb-3">
                    <div className="card p-3">
                        <div className="fw-bold mb-2">Monthly Production Output</div>
                        <Bar
                            data={{
                                labels: prodLabels,
                                datasets: [
                ...
                <div className="col-lg-3 mb-3">
                    <div className="card p-3">
                        <div className="fw-bold mb-2">Top Selling Products</div>
                        <Pie
                            data={{
                                labels: pieLabels,
                                datasets: [
                                    {
                                        data: pieData,
                                        backgroundColor: ['#8a5b2a', '#a7743f', '#c58b51', '#e0a86b', '#f2c48a']
                                    }
                                ]
                            }}
                ...
                <div className="col-lg-3 mb-3">
                    <div className="card p-3">
                        <div className="fw-bold mb-2">Low Stock Alerts</div>
                        <ul className="mb-0">
                            {(lowStock.raw_materials || []).slice(0, 5).map((m) => (
                                <li key={m.id}>
                                    {m.name}: {m.current_stock}/{m.minimum_stock}
                                </li>
                            ))}
                        </ul>
                    </div>
            ...
            <div className="row mb-3">
                <div className="col-lg-8 mb-3">
                    <div className="card p-3">
                        <div className="fw-bold mb-2">Sales Trend</div>
                        <Line
                            data={{
                                labels: salesLabels,
                                datasets: [
                                    {
                                        ...
                                        data: salesData,
                                        borderColor: '#8a5b2a',
                                        backgroundColor: 'rgba(138,91,42,0.2)'
                                    }
                                ]
                            }}
                            options={{ responsive: true }}
